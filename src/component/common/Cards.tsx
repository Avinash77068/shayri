...
export default function Cards() {
  ...
  return (
    ...
    <div
      className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 absolute  top-[5rem] pb-12  transition-all  duration-500 -z-10`}
    >
      {loading ? (
        ...
        <div className="text-center text-gray-600 text-2xl font-semibold flex items-center justify-center h-[60vh] w-full col-span-full">
          No Data Found
        </div>
      ) : (
        Alldata?.data?.map((val: values) => (
          <div
            key={val._id}
            className="p-4 border border-gray-200 rounded-2xl transition hover:shadow-lg bg-white hover:bg-slate-200 hover:border-purple-500 flex flex-col justify-between max-w-[300px]"
          >
    ...
    <FaTrashAlt onClick={() => handleDelete(val._id)} className="text-black w-6 h-6 hover:bg-black hover:text-white mt-1"/>
