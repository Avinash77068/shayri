...
export default function Sidebar() {
  ...
  return (
    <div className={`z-10 transition-transform duration-800 mt-[65px] p-2 bg-blue-400 text-white h-full flex-col gap-5 inline-flex border-r fixed w-[16rem] ${isOpened ? 'translate-x-0' : 'translate-x'}`}>
      {/* User Info */}
      <div className="w-full p-3 rounded-lg border border-gray-300 flex items-center justify-between">
        ...
        <button className="w-5 h-5">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path d="M10.0156 14.9896V15.0396M10.0156 9.97595V10.026M10.0156 4.96228V5.01228" stroke="black" strokeWidth="2.5" strokeLinecap="round" />
          </svg>
        </button>
      </div>
      {/* Menu */}
          ...
          {menuItems.map((item) => (
            <li key={item.label} className="hover:bg-green-100 rounded-lg">
              <Link to={item.to} className="flex p-3 bg-white rounded-lg items-center gap-3">
                {item.icon}
                <span className="text-black text-sm font-bold leading-snug">{item.label}</span>
              </Link>
            </li>
          ))}
        </ul>
